...
const MAX_BATCH_RUNS_PER_FULL_BACKFILL = 100
...
export const EMPTY_FIRST_HISTORICAL_PAGE_ERROR = 'empty_first_historical_page'
export const NO_HISTORICAL_DATA_AVAILABLE_ERROR = 'no_historical_data_available'
...
function createBackfillSummary({
  ...
  return {
    ...
    skipped: 0,
    jobStatus,
    endedDueToEmptyFirstPageBootstrap: false,
  }
}

function accumulateBackfillSummary(target, source) {
  if (!source) {
    return target
  }
  ...
  if (source.athleteId) {
    target.athleteId = source.athleteId
  }

  target.activitiesListed += toNonNegativeInteger(source.activitiesListed)
  target.activitiesScanned += toNonNegativeInteger(source.activitiesScanned)
  target.activitiesWithBestEfforts += toNonNegativeInteger(source.activitiesWithBestEfforts)
  target.fiveKilometerCandidates += toNonNegativeInteger(source.fiveKilometerCandidates)
  target.tenKilometerCandidates += toNonNegativeInteger(source.tenKilometerCandidates)
  target.halfMarathonCandidates += toNonNegativeInteger(source.halfMarathonCandidates)
  target.marathonCandidates += toNonNegativeInteger(source.marathonCandidates)
  target.detailedActivitiesFetched += toNonNegativeInteger(source.detailedActivitiesFetched)
  target.recordsUpdated += toNonNegativeInteger(source.recordsUpdated)
  target.skipped += toNonNegativeInteger(source.skipped)
  target.jobStatus = source.jobStatus ?? target.jobStatus
  target.endedDueToEmptyFirstPageBootstrap =
    target.endedDueToEmptyFirstPageBootstrap || source.endedDueToEmptyFirstPageBootstrap === true
...
function isPendingEmptyFirstHistoricalPageBootstrap(job) {
  return job.status === 'pending' && job.last_error === EMPTY_FIRST_HISTORICAL_PAGE_ERROR
}

function isCompletedNoHistoricalDataState(job) {
  return job.status === 'completed' && job.last_error === NO_HISTORICAL_DATA_AVAILABLE_ERROR
}
...
async function loadPersonalRecordBootstrapDataPresence(supabase, userId) {
  const [
    { count: personalRecordSourcesCount, error: personalRecordSourcesError },
    { count: personalRecordsCount, error: personalRecordsError },
  ] = await Promise.all([
    supabase
  ...
  if (personalRecordSourcesError) {
    throw new Error(personalRecordSourcesError.message)
  }
  ...
  return {
    hasPersonalRecordSources: (personalRecordSourcesCount ?? 0) > 0,
    hasPersonalRecords: (personalRecordsCount ?? 0) > 0,
  }
}

function isStaleCompletedBackfillBootstrap(job, bootstrapDataPresence) {
  return (
    job.status === 'completed'
    && job.last_error !== NO_HISTORICAL_DATA_AVAILABLE_ERROR
    && job.processed_activities_count === 0
    && job.scanned_pages_count === 0
    && bootstrapDataPresence.hasPersonalRecordSources === false
    && bootstrapDataPresence.hasPersonalRecords === false
  )
}
...
async function processConnection(supabase, connection, options) {
  ...
  let completed = false
  let endedDueToEmptyFirstPageBootstrap = false
  ...
  try {
    while (
      ...
      if (activities.length === 0) {
        ...
        if (isFirstPage && noProgress) {
          // no historical activity data available; treat as terminal completion.
          endedDueToEmptyFirstPageBootstrap = true
          completed = true
          break
        }
  ...
  if (completed) {
    if (!options.dryRun) {
      job = await updateBackfillJob(supabase, connection.user_id, {
        status: 'completed',
        last_error: endedDueToEmptyFirstPageBootstrap ? NO_HISTORICAL_DATA_AVAILABLE_ERROR : null,
    ...
    if (!options.dryRun) {
      job = await updateBackfillJob(supabase, connection.user_id, {
        status: 'pending',
        last_error: endedDueToEmptyFirstPageBootstrap ? EMPTY_FIRST_HISTORICAL_PAGE_ERROR : null,
        finished_at: null,
      })
    }
    ...
    console.info('Backfill batch ended before history completion; another run is needed', {
      ...
      endedDueToEmptyFirstPageBootstrap,
      dryRun: options.dryRun,
    })
  }
  ...
  summary.endedDueToEmptyFirstPageBootstrap = endedDueToEmptyFirstPageBootstrap
  return summary
}
...
export async function ensureHistoricalPersonalRecordBackfillForUser(userId, options = {}) {
  ...
  let job = await loadBackfillJob(supabase, normalizedUserId)
  const bootstrapDataPresence =
    job.created_at && job.status === 'completed'
      ? await loadPersonalRecordBootstrapDataPresence(supabase, normalizedUserId)
      : null
  const staleCompletedBootstrap =
    bootstrapDataPresence !== null
    && isStaleCompletedBackfillBootstrap(job, bootstrapDataPresence)
  let effectiveStatus = job.created_at ? job.status : 'missing'
  const cooldownActive = hasActiveConnectionRateLimitCooldown(connection)

  if (staleCompletedBootstrap) {
    console.warn('Resetting stale completed historical personal record backfill bootstrap', {
      userId: normalizedUserId,
      athleteId: connection.strava_athlete_id,
      job: formatBackfillJobStateForLog(job),
      hasPersonalRecordSources: bootstrapDataPresence.hasPersonalRecordSources,
      hasPersonalRecords: bootstrapDataPresence.hasPersonalRecords,
    })

    job = await updateBackfillJob(supabase, normalizedUserId, {
      status: 'pending',
      next_page: 1,
      processed_activities_count: 0,
      scanned_pages_count: 0,
  ...
  if (effectiveStatus === 'completed') {
    if (
      isCompletedNoHistoricalDataState(job)
      && options.ignoreEmptyFirstPageBootstrap === true
    ) {
      job = await updateBackfillJob(supabase, normalizedUserId, {
        status: 'pending',
        next_page: 1,
      ...
      return {
        ok: true,
        reason: isCompletedNoHistoricalDataState(job)
  ...
  if (
    effectiveStatus === 'pending'
    && isPendingEmptyFirstHistoricalPageBootstrap(job)
    && options.ignoreEmptyFirstPageBootstrap !== true
  ) {
    return {
      ok: true,
      reason: 'pending_empty_first_historical_page',
      triggered: false,
      jobStatus: 'pending',
    }
  }
  ...
  if (effectiveStatus === 'running') {
    if (isStaleRunningBackfillJob(job, STALE_RUNNING_BACKFILL_JOB_TIMEOUT_MS)) {
      const reclaimedJob = await reclaimStaleRunningBackfillJob(
        supabase,
        normalizedUserId,
        new Date(Date.now() - STALE_RUNNING_BACKFILL_JOB_TIMEOUT_MS).toISOString()
      ...
      if (reclaimedJob) {
        ...
        console.info('Reclaimed stale running backfill job before ensure early-return', {
          userId: normalizedUserId,
          athleteId: connection.strava_athlete_id,
          staleTimeoutMs: STALE_RUNNING_BACKFILL_JOB_TIMEOUT_MS,
          reclaimedStatus: job.status,
        ...
        job = await loadBackfillJob(supabase, normalizedUserId)
        effectiveStatus = job.created_at ? job.status : 'missing'
      }
    }
  }
  ...
  return {
    ok: true,
    reason:
      summary.endedDueToEmptyFirstPageBootstrap === true
        ? 'no_historical_data_available'
...
export async function runHistoricalPersonalRecordBackfillForUser(userId, options = {}) {
  const normalizedUserId = typeof userId === 'string' ? userId.trim() : ''
  ...
  const aggregateSummary = createBackfillSummary({
    userId: normalizedUserId,
    jobStatus: 'missing',
  })
  ...
  while (batchesRun < MAX_BATCH_RUNS_PER_FULL_BACKFILL) {
    const result = await ensureHistoricalPersonalRecordBackfillForUser(normalizedUserId, options)
    batchesRun += 1
    lastResult = result
    accumulateBackfillSummary(aggregateSummary, result.summary)

    if (result.reason === 'no_historical_data_available') {
      return {
        ...result,
        batchesRun,
        summary: accumulateBackfillSummary(
          createBackfillSummary({
            userId: normalizedUserId,
            jobStatus: result.jobStatus ?? aggregateSummary.jobStatus,
          }),
          aggregateSummary
        ),
      }
    ...
    if (!(result.ok === true && result.triggered === true && result.jobStatus === 'pending')) {
      return {
        ...result,
        batchesRun,
        summary: accumulateBackfillSummary(
  ...
  console.warn('Historical personal record backfill reached safety batch limit', {
    userId: normalizedUserId,
    batchesRun,
    lastReason: lastResult?.reason ?? null,
    lastJobStatus: lastResult?.jobStatus ?? aggregateSummary.jobStatus,
  })
  ...
  return {
    ...
    triggered: true,
    jobStatus: lastResult?.jobStatus ?? aggregateSummary.jobStatus,
    batchesRun,
...
export async function runHistoricalPersonalRecordBackfillScript(options = {}) {
  ...
  for (const connection of connections) {
    ...
    const result = await runHistoricalPersonalRecordBackfillForUser(connection.user_id, {
      dryRun,
    })

    if (result.ok !== true) {
      throw new Error(result.reason ?? 'historical_personal_record_backfill_failed')
    }

    const summary = accumulateBackfillSummary(
      createBackfillSummary({
        userId: connection.user_id,
        athleteId: connection.strava_athlete_id,
        jobStatus: result.jobStatus ?? 'missing',
      }),
      result.summary
    )
    summaries.push(summary)

    console.info('Finished user', {
      ...summary,
      reason: result.reason ?? null,
      triggered: result.triggered ?? false,
      batchesRun: result.batchesRun ?? 1,
    })
  }
