...
function readInitialMessagesListState(initialSeed: MessagesListPrefetchData | null) {
  if (initialSeed) {
    return initialSeed
  }
...
export default function MessagesPageClient({
  ...
  initialSeed: MessagesListPrefetchData | null
}) {
  ...
  const initialState = readInitialMessagesListState(initialSeed)
  const hasImmediateRenderData = initialState !== null
  const [loadingCommon, setLoadingCommon] = useState(() => initialState === null)
  const [currentUserId, setCurrentUserId] = useState<string | null>(() => initialState?.currentUserId ?? null)
  const [commonThreads, setCommonThreads] = useState<ClubThread[]>(() => initialState?.commonThreads ?? [])
  const [coachThread, setCoachThread] = useState<DirectCoachThreadItem | null>(
    () => initialState?.coachThread ?? null
  )
  const [directThreads, setDirectThreads] = useState<CoachDirectThreadItem[]>(
    () => initialState?.directThreads ?? []
  )
  const [students, setStudents] = useState<StudentProfile[]>(() => initialState?.students ?? [])
  const [unreadCountsByThread, setUnreadCountsByThread] = useState<UnreadCountsByThread>(
    () => initialState?.unreadCountsByThread ?? {}
  )
  ...
  const [deferredSectionsReady, setDeferredSectionsReady] = useState(() => initialState !== null)
  ...
  const shouldShowCommonSkeleton = !hasImmediateRenderData && loadingCommon && commonChatItems.length === 0
  const shouldShowDeferredSkeleton = !hasImmediateRenderData && (loadingCommon || !deferredSectionsReady)
  ...
  useEffect(() => {
    ...
    seedMessagesListCache(initialSeed)
    lastUnreadFetchAtRef.current = Date.now()
    setCurrentUserId(initialSeed.currentUserId)
    setCommonThreads(initialSeed.commonThreads)
    setCoachThread(initialSeed.coachThread)
    setDirectThreads(initialSeed.directThreads)
    setStudents(initialSeed.students)
    setUnreadCountsByThread(initialSeed.unreadCountsByThread)
  ...
  }, [applyUnreadCountsByThread, initialSeed])
  ...
  }, [currentUserId, scheduleUnreadCountsRefreshFromWindowAttention])

  useEffect(() => {
    if (!shouldLoadDeferredSections || !currentUserId || deferredSectionsReady) {
      return
    }
  ...
  useEffect(() => {
    if (!currentUserId || realtimeReady) {
      return
    }
