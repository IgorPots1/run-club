alter table public.race_week_results
add column if not exists race_bonus_xp integer;

alter table public.race_week_results
alter column race_bonus_xp set default 0;

update public.race_week_results
set race_bonus_xp = 0
where race_bonus_xp is null;

alter table public.race_week_results
alter column race_bonus_xp set not null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'race_week_results_race_bonus_xp_check'
      and conrelid = 'public.race_week_results'::regclass
  ) then
    alter table public.race_week_results
    add constraint race_week_results_race_bonus_xp_check
    check (race_bonus_xp >= 0);
  end if;
end
$$;

update public.race_week_results
set race_bonus_xp = case
  when rank = 1 then 120
  when rank <= 3 then 75
  when rank <= 10 then 40
  else 20
end
where race_bonus_xp is distinct from case
  when rank = 1 then 120
  when rank <= 3 then 75
  when rank <= 10 then 40
  else 20
...
begin
  ...
  scored_rows as (
    select
      ...
      s.challenge_xp,
      s.runs_count,
      (s.run_xp + s.like_xp + s.challenge_xp)::integer as total_xp,
      coalesce(
  ...
  select
    ir.user_id,
    case
      when ir.rank = 1 then 'race_week_winner'
      when ir.rank <= 3 then 'race_week_top_3'
      when ir.rank <= 10 then 'race_week_top_10'
      else null
    end as badge_code,
    p_race_week_id,
    'weekly_race',
...
revoke all on function public.finalize_race_week(uuid) from public;
revoke all on function public.finalize_race_week(uuid) from anon;
revoke all on function public.finalize_race_week(uuid) from authenticated;
