...
select
  ...
  uba.awarded_at,
  jsonb_strip_nulls(
    coalesce(uba.meta, '{}'::jsonb) || jsonb_build_object(
      'rank', uba.source_rank,
      'weekSlug', rw.slug
    )
  ) as meta
from public.user_badge_awards uba
left join public.race_weeks rw
  on rw.id = uba.race_week_id
where uba.source_type = 'weekly_race'
  and uba.race_week_id is not null
  and uba.source_rank between 1 and 3
on conflict (user_id, badge_code, race_week_id) do update
set
  source_type = excluded.source_type,
  source_rank = excluded.source_rank,
  meta = coalesce(user_badge_awards.meta, '{}'::jsonb) || excluded.meta;

delete from public.user_badge_awards uba
where uba.source_type = 'weekly_race'
  and (
    uba.badge_code in ('race_week_winner', 'race_week_top_3', 'race_week_top_10')
    or (
      uba.badge_code in ('weekly_race_1', 'weekly_race_2', 'weekly_race_3')
      and coalesce(uba.source_rank, 0) not between 1 and 3
    )
  );
